...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_type === "Student" || $user_type === "Teacher" || $user_type === "Parent") {
        ...
        switch ($user_type) {
            case "Student":
                $stmt_info = $conn->prepare("INSERT INTO student_info (firstname, lastname, username, gender, course, section, year_level) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt_info->bind_param("sssssss", $firstname, $lastname, $username, $genderS, $courseforstudent, $sectionforstudent, $yearlevelforstudent);
                break;
            case "Teacher":
        ...
        if (!$stmt_info->execute()) {
            die("Error inserting into specific info table: " . $stmt_info->error);
        }
